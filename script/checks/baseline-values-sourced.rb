...
Check.register(
  ...
  desc: "Every value in _data/baseline.yml's items: is an integer, a date, a boolean, or a well-formed TODO(owner: handle): marker",
  covers: ["3.1"]
) do |site|
  marker_re = /\ATODO\(owner:\s*\S[^)]*\):\s*\S/
  data = site.data.dig("baseline", "items")

  site.fail!("_data/baseline.yml's items: is missing or empty") if data.nil? || data.empty?
