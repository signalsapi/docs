...
Check.register(
  ...
  desc: "No file under _drafts/ appears in the built site or any machine-readable surface; each draft carries an owner and a sign-off marker",
  covers: ["9.7"]
) do |site|
  draft_paths = Dir.glob(File.join(ROOT, "_drafts", "*.md"))
  site.fail!("_drafts/ has no files") if draft_paths.empty?
